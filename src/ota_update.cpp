...
constexpr char kManifestUrl[] =
    "https://github.com/POG-Projects/pog-os-light/releases/latest/download/manifest.json";
constexpr char kReleaseAssetPrefix[] =
    "https://github.com/POG-Projects/pog-os-light/releases/download/v";
...
bool parseSemver(const String &raw, int &major, int &minor, int &patch) {
  ...
  if (first <= 0 || second <= first + 1 || version.indexOf('.', second + 1) >= 0) {
    return false;
  }
  ...
  for (char ch : a) if (!isDigit(ch)) return false;
  for (char ch : b) if (!isDigit(ch)) return false;
...
bool safeAssetName(const String &name) {
  if (!name.length() || name.length() > 80 || name.indexOf("..") >= 0) return false;
  for (char ch : name) {
    if (!(isAlphaNumeric(ch) || ch == '-' || ch == '_' || ch == '.')) return false;
  }
  return true;
}
...
esp_err_t httpEvent(esp_http_client_event_t *event) {
  ...
  if (context->text) {
    ...
    context->text->concat(static_cast<const char *>(event->data), event->data_len);
  ...
  if (context->sha) {
    ...
    context->received += event->data_len;
    if (context->total > 0) {
      uint8_t progress = min(99, static_cast<int>(
          context->received * 100ULL / static_cast<uint64_t>(context->total)));
...
esp_http_client_handle_t createHttpClient(const char *url, HttpContext *context) {
  esp_http_client_config_t config = {};
  config.url = url;
  config.user_agent = "PogLight/" POGLIGHT_FW_VERSION;
  ...
  // GitHub assets redirect to a signed URL whose path exceeds ESP-IDF's
...
bool fetchManifest(String &body, String &error) {
  ...
  if (context.tooLarge || !body.length()) {
    error = context.tooLarge ? "manifeste trop volumineux" : "manifeste vide";
    return false;
  }
  return true;
}
...
bool checkLatestRelease() {
  ...
  bool available = isNewerVersion(version, POGLIGHT_FW_VERSION);
  xSemaphoreTake(stateMutex, portMAX_DELAY);
  ...
  Serial.printf("[OTA] version locale %s, release %s%s\n", POGLIGHT_FW_VERSION,
                version.c_str(), available ? " disponible" : " à jour");
  return true;
}
...
bool installLatestRelease() {
  ...
  if (!client) {
    ...
    Update.abort();
    setPhase(Phase::Error, "initialisation du téléchargement impossible");
    return false;
  }
  ...
  if (result != ESP_OK || status != 200 || context.writeFailed || !context.received) {
    mbedtls_sha256_free(&sha);
    Update.abort();
...
void updaterTask(void *) {
  ...
  for (;;) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      if (checkRequested || (int32_t)(now - nextAutomaticCheck) >= 0) {
...
void otaUpdateBegin() {
  ...
  xTaskCreate(updaterTask, "ota-update", 12288, nullptr, 1, &updaterTaskHandle);
}
...
void otaUpdateFillJson(JsonObject out) {
  ...
  out["currentVersion"] = POGLIGHT_FW_VERSION;
  ...
  if (state.latestVersion.length()) {
    out["releaseUrl"] =
        String("https://github.com/POG-Projects/pog-os-light/releases/tag/v") +
        state.latestVersion;
  }
  xSemaphoreGive(stateMutex);
}
