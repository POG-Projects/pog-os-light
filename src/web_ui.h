...
.logo{position:relative;width:38px;height:38px;border-radius:13px;background:linear-gradient(145deg,rgba(255,255,255,.24),rgba(255,255,255,.04));border:1px solid rgba(255,255,255,.22);box-shadow:inset 0 1px 0 rgba(255,255,255,.28),0 10px 35px rgba(84,231,173,.13)}
.logo:before{content:"";position:absolute;inset:8px;border-radius:50% 50% 48% 52%;background:conic-gradient(from 210deg,var(--emerald),#9af5d2,var(--violet),var(--emerald));filter:blur(.2px);box-shadow:0 0 16px rgba(84,231,173,.55)}
...
.strip{display:flex;align-items:center;gap:3px;min-height:18px}
.strip>i{flex:1 1 0;aspect-ratio:1;max-height:14px;border-radius:50%;background:#20222b;box-shadow:inset 0 1px 1px rgba(255,255,255,.08);transition:background .3s,box-shadow .3s}
  ...
  <main class="layout">
    ...
    <div class="stack">
      <section class="card mobile-panel" id="viewHardware">
        ...
        <div class="row"><label>Écran SDA/SCL</label><select id="oledSwap"><option value="0">Normal · 13/11</option><option value="1">Inversé · 11/13</option></select></div>
        ...
        <div class="hint" id="oledStat"></div>
      </section>
...
function netInfo(s){
  ...
  if(o) o.innerHTML = s.oledFound
    ? ('Écran OLED · <span style="color:var(--emerald)">détecté</span> · 0x'+(s.oledAddr||0).toString(16)+(s.oledSwapped?' · câblage inversé':''))
    : '<span style="color:var(--danger)">Écran OLED non détecté</span> · vérifiez VCC, GND, SDA et SCL';
}
async function load(){st=await api('/api/state');if(st.board=='esp32c3')PINS=[2,3,4,5,6,7,10];else if(st.board=='esp32')PINS=[2,4,5,12,13,14,16,17,18,19,21,22,23];fill(st.config);netInfo(st);const preview=new URLSearchParams(location.search).has('onboarding');if(preview||(st.apMode&&!onboardingDone()))showOnboarding(st.config)}
load();
